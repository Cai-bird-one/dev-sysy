...
本文档只描述当前编译器新增的 Koopa tensor IR dialect。优化相关内容统一放在 `OPTIMIZATION.md`。
...
- RISC-V 后端不会把 tensor dialect 行当作普通标量指令处理。
- Tensor 规整通过 module pass 参与 IR 优化 pipeline。
...
- `src/compiler/ir/opt/tensor/koopa_tensor_optimize.cpp`
- `src/compiler/ir/opt/tensor/koopa_tensor_optimize.h`
- `src/compiler/lexer/token_rules.cpp`
- `src/compiler/parser/grammar_rules.cpp`
- `src/compiler/ir/opt/model/ir_module.h`
- `src/compiler/ir/opt/passes/tensor_dialect_optimization.cpp`
...
- Tensor IR 当前只记录元信息，不会把 tensor 操作 lowering 成实际循环代码或专用指令。
...
- `add_relu`、`matmul_bias`、`matmul_relu`、`matmul_bias_relu` 及其 `_ta/tb` 变体允许作为 tensor dialect 行直接出现并校验，但不会在后端生成专用 tensor 指令。
