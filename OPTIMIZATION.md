...
- 目标寄存器直写和直接比较分支折叠在函数发射阶段始终启用。线上验证表明，按“分支密集函数”粗粒度关闭这两类优化会让 `mm/mv/spmv/brainfuck` 明显退化；布局保护只应用在汇编级会移动 label 或分支目标的窥孔改写上。
