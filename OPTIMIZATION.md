...
- 当前线上测试配置采用激进策略：删除相邻的重复 `mv a, b; mv a, b` 和往返 `mv a, b; mv b, a`。
- 当前线上测试配置采用激进策略：将 `bnez/beqz L; j F; L:` 改写为反向条件分支，减少一条无条件跳转。
- 保守布局敏感策略仍保留在代码中作为备选分析逻辑，但本轮未启用；因此分支密集或超大函数也会参与上述删除和重排。
...
| 当前工作区 | `19/19 AC` | `197.725312s` | IR pipeline 上限提升到 128 轮，RISC-V 窥孔优化改为固定点迭代；`brainfuck-bootstrap` 三跑均值为 `14.826351s` |

相对 `1b95318`，当前工作区在该子集上减少 `16.582435s`，约 `7.7%`；相对 `3d835cdabc52`，减少 `74.974552s`，约 `27.5%`。继续分析实际生成的 `brainfuck-bootstrap` 汇编后，发现 `cmp -> ne/eq cmp, 0 -> br` 会产生额外 `snez/mv`，保守阶段曾采用“只删除冗余布尔化、不改写为直接条件分支”的策略。该策略让 `brainfuck-bootstrap` 从上一轮单跑 `16.353877s` 降到三跑均值 `15.141303s`。多候选线性扫描后，完整 19 用例单跑总时间为 `200.824968s`；本轮 RISC-V 窥孔固定点迭代后进一步到 `197.725312s`，`brainfuck-bootstrap` 三跑均值为 `14.826351s`，未发现正确性回归。

本轮为了线上测试重新启用此前在部分测试集有效、但本地 2021 子集上不稳定的激进后端优化：
...
针对 `brainfuck-bootstrap` 的负优化分析表明，它的热点是分支密集解释器：`get_bf_char` 约 `178` 条指令含 `19` 条分支，`read_program` 约 `104` 条指令含 `8` 条分支，`run_program` 约 `2953` 条指令含 `41` 条分支。直接删除冗余 `mv` 虽然减少静态指令数，但会移动后续 label 和分支目标地址，使 qemu 的 translation block 形状、分支 chaining 和 host code cache 布局发生变化，收益被布局扰动抵消甚至放大为稳定退化。保守策略会把这类函数标记为布局敏感，只做不改变 guest 指令位置的 `mv -> nop`；当前线上测试配置改为无保护激进模式，用于观察线上评测机和隐藏用例是否更受益于静态指令减少。
