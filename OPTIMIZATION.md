...
- 对分支密集或超大函数，函数发射阶段会关闭上述“目标寄存器直写”和直接比较分支折叠，回到经 `t0/t1` 中转的保守形态，避免局部删除 `mv/li` 后改变热点分支布局造成负优化。
...
- 对分支密集或超大函数启用布局敏感保护：不删除跳转到紧邻 label 的指令，而是改写为 `nop`；不做分支反转；相邻冗余 `mv` 也改写为 `nop`。这样保留 guest 指令位置和 label 距离，避免热点解释器、大型 if-chain 或 qemu translation block 对布局变化过敏。
