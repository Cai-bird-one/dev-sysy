...
6. 后端使用 IR 层活跃变量分析结果做多候选线性扫描寄存器分配。
7. 指令发射阶段使用更紧凑的 RISC-V 指令选择。
8. 最终汇编经过 RISC-V 窥孔优化器；窥孔优化同样最多迭代 128 轮，直到汇编不再变化。
...
- 若比较结果只被紧随其后的 `eq/ne %cmp, 0/1` 和 `br` 使用，函数发射阶段会跳过这次冗余布尔化，让 `br` 直接读取原始比较结果；这保留 `slt/seqz + bnez` 的 qemu 友好形态，避免直接改成 `beq/blt` 带来的负收益。
...
RISC-V 窥孔优化位于 `src/compiler/riscv/opt/peephole_optimizer.cpp`，在完整汇编生成后运行。优化器会把现有局部规则反复执行到固定点，最多 128 轮，因此前一轮产生的 `mv`、简化算术或跳转布局变化可以继续暴露给后一轮处理。
...
相对 `1b95318`，当前工作区在该子集上减少 `16.582435s`，约 `7.7%`；相对 `3d835cdabc52`，减少 `74.974552s`，约 `27.5%`。继续分析实际生成的 `brainfuck-bootstrap` 汇编后，发现 `cmp -> ne/eq cmp, 0 -> br` 会产生额外 `snez/mv`，因此加入了只删除冗余布尔化、不改写为直接条件分支的保守优化。该优化让 `brainfuck-bootstrap` 从上一轮单跑 `16.353877s` 降到三跑均值 `15.141303s`。多候选线性扫描后，完整 19 用例单跑总时间为 `200.824968s`；本轮 RISC-V 窥孔固定点迭代后进一步到 `197.725312s`，`brainfuck-bootstrap` 三跑均值为 `14.826351s`，未发现正确性回归。

已回退的负收益或不稳定后端实验包括：

- `mul x, 2^k +/- 1` 展开为移位加减。
- 常量条件分支直接发射。
- 将比较结果直接改写为 `beq/bne/blt/bge` 分支；`brainfuck-bootstrap` 虽然 AC，但三跑稳定退化到约 `43s`。
- 相邻反向 `mv` 删除；语义正确但同一用例三跑稳定退化到约 `43s`，已回退。
- RISC-V 窥孔阶段比较分支折叠。
