...
本文档描述当前编译器实现的优化。优化覆盖 tensor 注释规整、Koopa IR pass pipeline、RISC-V 后端代码生成优化、窥孔优化，以及基于活跃变量分析的线性扫描寄存器分配。

## 1. 优化入口

`-perf` 模式下的主要优化流程如下：

1. `IrOptimizer` 解析并规整 tensor 注释。
2. `IrOptimizer` 解析 Koopa IR，并反复运行 `buildPerfPipeline()`，最多迭代 8 轮，直到 pass 不再产生变化。
3. RISC-V 后端解析优化后的 Koopa IR。
4. 后端使用 IR 层活跃变量分析结果做线性扫描寄存器分配。
5. 指令发射阶段使用更紧凑的 RISC-V 指令选择。
6. 最终汇编经过 RISC-V 窥孔优化器。
...
Tensor 注释优化位于 `src/compiler/ir/koopa_tensor_optimize.cpp`。它只处理 `// tensor ...` 注释中的元信息，不改变普通 Koopa 指令。
...
- 删除不再被其他 tensor 注释依赖、且不需要作为最终结果保留的中间 tensor 操作。
...
// tensor %bias: tensor<1x4>
// tensor %bb = broadcast %bias : tensor<2x4>
// tensor %out = matmul_bias_relu %a, %b, %bb : tensor<2x4>
```
...
1. `UnusedFunctionEliminationPass`
2. `ConstantFoldingPass`
3. `AlgebraicSimplifyPass`
4. `GlobalValuePropagationPass`
5. `LocalLoadStoreForwardingPass`
6. `GlobalScalarLoadForwardingPass`
7. `LocalDeadStoreEliminationPass`
8. `CommonSubexpressionEliminationPass`
9. `LoopInvariantCodeMotionPass`
10. `BranchSimplifyPass`
11. `JumpThreadingPass`
12. `UnreachableBlockEliminationPass`
13. `DeadCodeEliminationPass`
14. `UnusedAllocEliminationPass`
15. `BlockCleanupPass`
...
- Tensor 注释：`src/tests/compiler/ir/koopa_tensor_extension.cpp`
...
相对当前优化前基线，最终耗时减少 `56.19s`，约为原来的 `81.0%`，即约 `1.23x` 加速。相对 `3d835cd` 旧基线，最终从 `455/467` 提升到 `467/467`，整套测试 wall time 约为旧基线的 `21.3%`。

这些 `real time` 是本地 Docker 单次完整测试的墙钟时间，包含编译、运行、失败或超时等待等因素，不等同于评测平台单独的性能分数；但它们能反映当前实现对完整测试集的通过能力和总体执行效率。
